...
internal class Grid {
    ...
    fileprivate var majorGridColor: Color {
        ...
        set {
            backingView.majorGridColor = newValue
        }
    }
...
internal class GridView: UIView {
    ...
    fileprivate var majorGridColor = Color(uiColor: UIColor(named: "majorGrid")?.withAlphaComponent(0.2) ?? UIColor(white: 0.85, alpha: 1.0)) {
        didSet {
            setNeedsDisplay()
        }
    }
    
    fileprivate var minorGridColor = Color(uiColor: UIColor(named: "minorGrid")?.withAlphaComponent(0.1) ?? UIColor(white: 0.95, alpha: 1.0)) {
        didSet {
            setNeedsDisplay()
        }
    }
